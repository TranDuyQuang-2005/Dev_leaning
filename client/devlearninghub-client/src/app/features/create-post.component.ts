...
export class CreatePostComponent implements OnInit{
  ...
  ngOnInit(){
    ...
    if(id){
      ...
      this.api.get<any>(`/api/v1/forum/posts/${this.postId}`).subscribe({
        next: (r: any) =>{const p=r.data;this.attachments=p.attachments||[];this.form={title:p.title,content:p.content,tags:(p.tags||[]).map((x:any)=>x.name),attachmentIds:this.attachments.map((x:any)=>x.fileId)};},
        error: (e: any) =>this.error=e?.error?.message||'KhÃ´ng táº£i Ä‘Æ°á»£c bÃ i viáº¿t'
      });
    }
  }
  ...
  onFilesSelected(event:any){
    ...
    if(this.attachments.length+files.length>5){this.error='Tá»‘i Ä‘a 5 file/hÃ¬nh cho má»—i bÃ i viáº¿t';event.target.value='';return;}
    ...
    Array.from(files).forEach(file=>{
      ...
      this.api.upload<any>('/api/v1/forum/uploads',formData).subscribe({
        next: (r: any) =>{const a=r.data;this.attachments.push(a);this.form.attachmentIds=this.attachments.map(x=>x.fileId);},
        error: (e: any) =>{this.error=e?.error?.message||`Upload tháº¥t báº¡i: ${file.name}`;},
  ...
  save(){
    this.error='';
    if(!this.form.title||this.form.title.trim().length<10){this.error='TiÃªu Ä‘á» pháº£i tá»« 10 kÃ½ tá»±';return;}
    if(!this.form.content||this.form.content.trim().length<20){this.error='Ná»™i dung pháº£i tá»« 20 kÃ½ tá»±';return;}
    if(!this.form.tags.length){this.error='Cáº§n Ã­t nháº¥t 1 tag';return;}
    ...
    req.subscribe({next: (r: any) =>{this.loading=false;this.router.navigate(['/learner/forum-post',r.data.id]);},error: (e: any) =>{this.loading=false;this.error=e?.error?.message||'KhÃ´ng lÆ°u Ä‘Æ°á»£c bÃ i viáº¿t';}})
  }
}
